  ...
  <div class="container">
    ...
    <div class="form-group">
      ...
      <input
        ...
        name=""
        id="videoCreateTime"
        @change="getVideoDataReleaseTime"
      />
      <br />
      <button
        type="submit"
        class="btn btn-primary"
        id="btn"
        @click.prevent="PostDataAllReleaseTime"
      >
        发送视频创作时间
      </button>
    </div>
    ...
    <div class="form-group">
      ...
      <button
        ...
        class="btn btn-primary"
        id="btn"
        @click.prevent="PostVideoCommentData"
      >
        发送视频评论数据
      </button>
    </div>
  ...
  methods: {
    ...
    matching() {
      ...
      for (let i = 0; i < imgarr.length; i++) {
        ...
        for (let j = 0; j < novelArr.length; j++) {
          if (
            sign ===
            novelArr[j].name.substring(0, novelArr[j].name.indexOf("."))
          ) {
    ...
    async uploadImg(index) {
      ...
      const { data: res } = await Axios.post(
        "http://localhost:8000/storageServices/picture/upload",
        formData,
        {
    ...
    async uploadVideo(index) {
      ...
      const { data: res } = await Axios.post(
        "http://localhost:8000/storageServices/video/upload",
        formData,
        {
          "Content-type": "multipart/form-data",
        }
      ...
      reader.onload = () => {
        ...
        for (let i = 0; i < content.length; i++) {
          ...
          if (videoTemp[3].indexOf("w") != -1) {
            videoTemp[3] = parseFloat(
              videoTemp[3].substr(0, videoTemp[3].length - 1) * 10000
            );
    ...
    getVideoDataReleaseTime() {
      ...
      const reader = new FileReader();
      reader.readAsText(
        document.getElementById("videoCreateTime").files[0],
        "utf8"
    ...
    getVideoCommentData() {
      const reader = new FileReader();
      reader.readAsText(
        document.getElementById("videoComment").files[0],
        "utf8"
      ); // input.files[0]为第一个文件
      reader.onload = () => {
        let content = reader.result;
    ...
    PostVideoCommentData() {
      for (let i = 0; i < this.commentData.length; i++) {
        ...
        obj1.avatar_uri =
          "https://p3-pc.douyinpic.com/aweme/100x100/" + obj1.avatar_uri;
